<script>
  ...
  async function load() {
    ...
    try {
      ...
      rows = await api(`/messages?${params}`);
    } catch (e) {
      error = e.message;
    }
  }
  ...
  <div class="card bg-base-100 border border-base-300">
    <div class="card-body gap-3">
      <div class="flex items-start justify-between gap-2">
        ...
        <button class="btn btn-sm" onclick={() => (detail = null)}>← Back</button>
      </div>
      ...
      {:else}
        <div class="overflow-x-auto">
          <table class="table table-sm">
            ...
            <tbody>
              {#each rows as m (m.id)}
                <tr class="hover cursor-pointer" onclick={() => open(m.id)}>
